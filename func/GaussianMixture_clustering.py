...
def find_optimal_GaussianMixture(
    X,
    k_range=range(2, 11),
) -> pd.DataFrame:
    """
    Поиск оптимального количества кластеров с помощью GaussianMixtureClustering.
    ...
    for k in k_range:
        ...
        labels = GaussianMixture_model.fit_predict(X)
...
def plot_GaussianMixture_metrics(results_df: pd.DataFrame) -> None:
    """
    Визуализирует метрики качества для GaussianMixture Clustering.
    """
...
__all__ = ['find_optimal_GaussianMixture', 'plot_GaussianMixture_metrics']
